...
- v1.10.6 : detection "chaine hors-ligne" unifiee et plus robuste, partagee par le watchdog, l'auto-switch et le reloader (une seule source de verite). Garde principale : tant qu'un direct HLS tourne (meme mis en pause en arriere-plan), la chaine n'est jamais vue hors-ligne -> le watchdog peut toujours relancer un live fige (plus de farm perdu sur faux positif). L'auto-switch ne bascule plus que sur 2 confirmations consecutives, avec compteur remis a zero au changement de chaine (anti-bascule pendant un raid) et re-verification juste avant de partir (ne quitte pas une chaine redevenue en direct). Couverture de tests etendue (detection hors-ligne, auto-switch, non-accumulation du watchdog).
- v1.10.5 : nouveau reglage "Vider l historique apres (min)" dans l'onglet Reglages. Purge automatiquement les entrees de l'historique plus vieilles que la duree choisie ; champ vide ou 0 = jamais (rien n'est efface). La purge s'applique a chaque nouveau claim et a l'ouverture du popup.
- v1.10.4 : corrige les noms de drops dans l'historique. Le verbe d'action (\"Recuperer\", \"Reclamer\"...) etait parfois capte depuis le bouton de reclamation (bandeau sur un stream) et collait au nom (ex. \"Recuperer Shooting Star\"). Il est desormais retire, pour les nouveaux drops comme retroactivement a l'affichage de l'historique deja enregistre.
- v1.10.3 : nouveau filet de securite "watchdog" (sous le toggle Reload auto) : sur un onglet de chaine en arriere-plan, si le lecteur reste fige plus de 10 min alors que l'anti-pause n'arrive pas a le relancer (et que la chaine n'est pas hors-ligne), la page est rechargee automatiquement. Garde anti-boucle : abandon apres 3 reloads consecutifs sans reprise de la lecture (compteur remis a zero des que ca rejoue).
- v1.10.2 : l'inventaire se recharge plus souvent (toutes les 3 min au lieu de 5) et immediatement apres qu'un drop a ete reclame depuis le bandeau d'un stream (l'onglet inventaire ouvert est recharge par le service worker pour remettre a jour les barres de progression, avec un throttle de 30 s anti-rafale).
